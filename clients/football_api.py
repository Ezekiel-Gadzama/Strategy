...
class APIFootballClient:
            ...
            try:
                ...
                if isinstance(filtered_event_dict.get('event_type'), str):
                    ...
                    event_type_mapping = {
                        ...
                        'shot on target': EventType.SHOT_ON_TARGET,
                        'shot': EventType.SHOTS,
                        'penalty awarded': EventType.PENALTY_AWARDED,
                        ...
                        'shots': EventType.SHOTS,
                        'penalty_awarded': EventType.PENALTY_AWARDED,
    ...
    def _classify_event_type(self, event_data: Dict[str, Any]) -> Optional[EventType]:
        ...
        event_type_mapping = {
            ...
            'Second Yellow': EventType.SECOND_YELLOW,
            'Corner': EventType.CORNER,
            'Substitution': EventType.TEAM_STATS,
            'Var': EventType.TEAM_STATS,
            'Foul': EventType.FOULS,
            'Offside': EventType.OFFSIDES,
            'Shot on Target': EventType.SHOT_ON_TARGET,
        ...
        # Handle card details for second yellow
        if event_type == 'Card' and detail == 'Second Yellow card':
            return EventType.SECOND_YELLOW
    ...
    def _get_event_description(self, event_data: Dict[str, Any]) -> str:
        ...
        if event_type == 'Goal':
            if detail == 'Normal Goal':
                return f"Goal by {player} ({team})"
            elif detail == 'Own Goal':
                return f"Own goal by {player} ({team})"
            elif detail == 'Penalty':
                return f"Penalty goal by {player} ({team})"
        elif event_type in ['Yellow Card', 'Red Card', 'Second Yellow']:
            return f"{event_type} for {player} ({team})"
        elif event_type == 'Corner':
            return f"Corner kick for {team}"
        elif event_type == 'Shot on Target':
            return f"Shot on target by {player} ({team})"
        elif event_type == 'Penalty Awarded':
            return f"Penalty awarded to {team}"
        elif event_type in ['Throw-in', 'Free Kick', 'Goal Kick']:
            return f"{event_type} for {team}"

        return f"{event_type}: {detail} ({team})"
    ...
    def _extract_half_stats(self, match: Match, events_data: List[Dict[str, Any]]):
        ...
        for event in events_data:
            ...
            # Count cards - now with specific card types
            elif event.get('type') in ['Yellow Card', 'Red Card', 'Second Yellow']:
                if is_home:
                    half.home_cards += 1
                else:
                    half.away_cards += 1
    ...
    def _generate_derived_events(self, match: Match):
        ...
        # Total cards with specific types
        total_yellows = ((match.home_stats.yellow_cards if match.home_stats else 0) +
                         (match.away_stats.yellow_cards if match.away_stats else 0))
        total_reds = ((match.home_stats.red_cards if match.home_stats else 0) +
                      (match.away_stats.red_cards if match.away_stats else 0))

        match.events.append(MatchEvent(
            event_type=EventType.CARDS,
            value=total_yellows + total_reds,
        ...
        # Penalty awarded events
        penalty_events = [e for e in match.events if e.event_type == EventType.PENALTY_AWARDED]
        if penalty_events:
            match.events.append(MatchEvent(
                event_type=EventType.PENALTY_AWARDED,
                value=len(penalty_events),
                description="penalties_awarded"
            ))

        # Add shot on target events
        shot_on_target_events = [e for e in match.events if e.event_type == EventType.SHOT_ON_TARGET]
        if shot_on_target_events:
            match.events.append(MatchEvent(
                event_type=EventType.SHOT_ON_TARGET,
                value=len(shot_on_target_events),
                description="total_shots_on_target"
            ))
