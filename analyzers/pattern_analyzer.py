...
class PatternAnalyzer:
    ...
    def _analyze_matches_optimized(self, matches: List[Match], league_id: int, season: int, league_name: str) -> Dict[
        ...
        # 🔥 FIX: Don't use batch processing for small numbers of matches
        if total_matches <= 30:  # Adjust threshold as needed
            print("🔄 Using single-batch processing for small dataset")
            self._process_batch_with_global_dict(matches)
        else:
            # Process matches in batches using global dictionary (only for large datasets)
            ...
            for batch_start in range(0, len(matches), batch_size):
                ...
                print(
                    f"✅ Batch {batch_count} complete: {len(self.global_combinations_dict)} unique combinations so far")

        # No combining needed - we already have the final results in global_combinations_dict!
        print(f"\n🎉 All batches processed")

        # 🔥 FIX: Count occurrences correctly - each combination should only be counted once per match
        # But currently your counting logic is flawed
        ...
        for combo_key, count in self.global_combinations_dict.items():
            ...
            # 🔥 FIX: The count should not exceed total_matches
            final_combinations_with_tuples[combo_tuple] = min(count, total_matches)
        ...
        final_results = self._prepare_final_results_lazy(final_combinations_with_tuples, total_matches,
                                                         len(final_combinations_with_tuples), league_id, season)
    ...
    def _process_batch_with_global_dict(self, matches: List[Match]):
        ...
        # Don't split into sub-batches if we have few matches
        if len(matches) <= self.thread_count * 30:
            # Process all matches in a single batch
            self._analyze_match_batch_global(matches)
        else:
            # Split batch for threading (only for larger batches)
    ...
    def _prepare_final_results_lazy(
        ...
        if not self.use_odd_info:
            ...
            stats = self._calculate_stats_from_raw(all_occurred_candidates, all_never_occurred_candidates,
                                                   total_matches)
